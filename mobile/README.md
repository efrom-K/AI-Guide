...
an **OpenStreetMap** map, and lets you **ask by voice or text**.
...
- **Spoken narration** — on-device TTS (`flutter_tts`, ru-RU). Speaker toggle in the app bar;
  asking (voice or text) hushes the guide (barge-in).
...
3. In the app: **Подключиться** → **▶ Прогулка** (or flip **GPS** for real positions) →
   the guide narrates and the map follows you; tap the mic or type to ask.
...
`path_provider` (mic), `flutter_map` + `latlong2` (OpenStreetMap).
